import { r as registerInstance, c as createEvent, f as forceUpdate, h, H as Host, g as getElement } from './index-eb209d91.js';
import { C as CSS_UTILITY } from './resources-f0858a17.js';
import { g as getElementDir, c as getSlotted, f as focusElement } from './dom-8588618b.js';
...
const CalciteAction = class {
  ...
  render() {
    ...
    return (h(Host, null, h("button", { "aria-busy": loading.toString(), "aria-disabled": disabled.toString(), "aria-label": ariaLabel, class: buttonClasses, disabled: disabled, onClick: this.buttonClick, ref: (buttonEl) => (this.buttonEl = buttonEl) }, this.renderIconContainer(), this.renderTextContainer())));
  }
...
const CalciteActionBar = class {
  constructor(hostRef) {
    ...
    this.resize = (height) => {
      ...
      const groupCount = getSlotted(el, SLOTS$3.bottomActions) || !expandDisabled
  ...
  renderBottomActionGroup() {
    ...
    const tooltip = getSlotted(el, SLOTS$3.expandTooltip);
    ...
    return getSlotted(el, SLOTS$3.bottomActions) || expandToggleNode ? (h("calcite-action-group", { class: CSS$2.actionGroupBottom }, h("slot", { name: SLOTS$3.bottomActions }), h("slot", { name: SLOTS$3.expandTooltip }), expandToggleNode)) : null;
  }
  render() {
...
const CalciteActionGroup = class {
  ...
  renderTooltip() {
    const { el } = this;
    const hasTooltip = getSlotted(el, SLOTS$2.menuTooltip);
    return hasTooltip ? h("slot", { name: SLOTS$2.menuTooltip, slot: SLOTS$1.tooltip }) : null;
  }
  renderMenu() {
    const { el, expanded, intlMore, menuOpen } = this;
    const hasMenuItems = getSlotted(el, SLOTS$2.menuActions);
    return hasMenuItems ? (h("calcite-action-menu", { expanded: expanded, flipPlacements: ["left", "right"], label: intlMore || TEXT$1.more, onCalciteActionMenuOpenChange: this.setMenuOpen, open: menuOpen, placement: "leading-start" }, this.renderTooltip(), h("slot", { name: SLOTS$2.menuActions }))) : null;
  }
  render() {
...
const CalciteActionMenu = class {
  constructor(hostRef) {
    ...
    this.setTooltipReferenceElement = () => {
      ...
      const slotted = getSlotted(el, SLOTS$1.tooltip);
  ...
  renderMenuButton() {
    ...
    const actionNode = (h("calcite-action", { active: open, "aria-controls": menuId, "aria-expanded": open.toString(), "aria-haspopup": "true", class: CSS$1.menuButton, icon: ICONS.menu, id: menuButtonId, label: label, onClick: this.menuButtonClick, onKeyDown: this.menuButtonKeyDown, onKeyUp: this.menuButtonKeyUp, ref: this.setMenuButtonRef, text: label, textEnabled: expanded }));
    return getSlotted(el, SLOTS$1.tooltip) ? (h("calcite-tooltip-manager", null, actionNode)) : (actionNode);
  }
  renderMenuItems() {
    ...
    return (h("calcite-popover", { label: label, open: open, placement: placement, referenceElement: menuButtonEl }, h("div", { "aria-activedescendant": activeDescendantId, "aria-labelledby": menuButtonId, class: CSS$1.menu, id: menuId, onKeyDown: this.menuActionsContainerKeyDown, onKeyUp: this.menuActionsContainerKeyUp, ref: (el) => (this.menuEl = el), role: "menu", tabIndex: -1 }, h("slot", null))));
  }
  render() {
    return (h(Host, null, this.renderMenuButton(), this.renderMenuItems(), h("slot", { name: SLOTS$1.tooltip })));
  }
