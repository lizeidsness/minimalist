...
import { c as getSlotted, g as getElementDir } from './dom-8588618b.js';
import { C as CalciteHeading } from './CalciteHeading-1a98987c.js';
...
const CalciteBlock = class {
  ...
  renderScrim() {
    ...
    return [
      loading || disabled ? h("calcite-scrim", { loading: loading, theme: theme }) : null,
      defaultSlot
    ];
  }
  render() {
    ...
    const headerContent = (h("header", { class: CSS$1.header }, hasIcon ? (h("div", { class: CSS$1.icon }, h("slot", { name: SLOTS.icon }))) : null, h("div", { class: CSS$1.title }, h(CalciteHeading, { class: CSS$1.heading, level: headingLevel }, heading), summary ? h("div", { class: CSS$1.summary }, summary) : null)));
    const hasControl = getSlotted(el, SLOTS.control);
    const headerNode = (h("div", { class: CSS$1.headerContainer }, this.dragHandle ? h("calcite-handle", null) : null, collapsible ? (h("button", { "aria-label": toggleLabel, class: CSS$1.toggle, onClick: this.onHeaderClick, title: toggleLabel }, headerContent)) : (headerContent), loading ? (h("calcite-loader", { inline: true, "is-active": true, label: intlLoading })) : hasControl ? (h("div", { class: CSS$1.controlContainer }, h("slot", { name: SLOTS.control }))) : null));
