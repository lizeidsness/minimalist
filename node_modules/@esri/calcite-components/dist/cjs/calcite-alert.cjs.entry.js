...
const interfaces = require('./interfaces-bc0f5779.js');
...
const CalciteAlert = class {
  ...
  render() {
    ...
    const closeButton = (index.h("button", { "aria-label": this.intlClose, class: "alert-close", onClick: this.closeAlert, ref: (el) => (this.closeButton = el), type: "button" }, index.h("calcite-icon", { icon: "x", scale: "m" })));
    const queueCount = (index.h("div", { class: `${this.queueLength > 1 ? "active " : ""}alert-queue-count` }, "+", this.queueLength > 2 ? this.queueLength - 1 : 1));
    const { active } = this;
    const progress = index.h("div", { class: "alert-dismiss-progress" });
    const role = this.autoDismiss ? "alert" : "alertdialog";
    const hidden = !active;
    return (index.h(index.Host, { active: active, "aria-hidden": hidden.toString(), "aria-label": this.label, "calcite-hydrated-hidden": hidden, dir: dir, queued: this.queued, role: role }, this.requestedIcon ? (index.h("div", { class: "alert-icon" }, index.h("calcite-icon", { icon: this.requestedIcon, scale: "m" }))) : null, index.h("div", { class: "alert-content" }, index.h("slot", { name: "alert-title" }), index.h("slot", { name: "alert-message" }), index.h("slot", { name: "alert-link" })), queueCount, !this.autoDismiss ? closeButton : null, this.active && !this.queued && this.autoDismiss ? progress : null));
  }
