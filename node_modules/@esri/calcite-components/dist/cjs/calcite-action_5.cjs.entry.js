...
const CalciteAction = class {
  ...
  renderIconContainer() {
    ...
    return hasIconToDisplay ? (index.h("div", { "aria-hidden": "true", class: CSS$3.iconContainer, key: "icon-container" }, iconNode, slotContainerNode)) : null;
  }
  render() {
    ...
    return (index.h(index.Host, null, index.h("button", { "aria-busy": loading.toString(), "aria-disabled": disabled.toString(), "aria-label": ariaLabel, class: buttonClasses, disabled: disabled, onClick: this.buttonClick, ref: (buttonEl) => (this.buttonEl = buttonEl) }, this.renderIconContainer(), this.renderTextContainer())));
  }
...
const CalciteActionBar = class {
  constructor(hostRef) {
    ...
    this.resize = (height) => {
      const { el, expanded, expandDisabled, lastActionCount, lastGroupCount, lastResizeHeight } = this;
      ...
      const groupCount = dom.getSlotted(el, SLOTS$3.bottomActions) || !expandDisabled
  ...
  renderBottomActionGroup() {
    ...
    const tooltip = dom.getSlotted(el, SLOTS$3.expandTooltip);
    ...
    return dom.getSlotted(el, SLOTS$3.bottomActions) || expandToggleNode ? (index.h("calcite-action-group", { class: CSS$2.actionGroupBottom }, index.h("slot", { name: SLOTS$3.bottomActions }), index.h("slot", { name: SLOTS$3.expandTooltip }), expandToggleNode)) : null;
  }
  render() {
...
const CalciteActionGroup = class {
  ...
  renderTooltip() {
    const { el } = this;
    const hasTooltip = dom.getSlotted(el, SLOTS$2.menuTooltip);
    return hasTooltip ? index.h("slot", { name: SLOTS$2.menuTooltip, slot: SLOTS$1.tooltip }) : null;
  }
  renderMenu() {
    const { el, expanded, intlMore, menuOpen } = this;
    const hasMenuItems = dom.getSlotted(el, SLOTS$2.menuActions);
    return hasMenuItems ? (index.h("calcite-action-menu", { expanded: expanded, flipPlacements: ["left", "right"], label: intlMore || TEXT$1.more, onCalciteActionMenuOpenChange: this.setMenuOpen, open: menuOpen, placement: "leading-start" }, this.renderTooltip(), index.h("slot", { name: SLOTS$2.menuActions }))) : null;
  }
  render() {
...
const CalciteActionMenu = class {
  constructor(hostRef) {
    ...
    this.setTooltipReferenceElement = () => {
      ...
      const slotted = dom.getSlotted(el, SLOTS$1.tooltip);
    ...
    this.getActions = () => {
      var _a;
      const { el } = this;
      const assignedElements = (_a = el
        .querySelector("slot")) === null || _a === void 0 ? void 0 : _a.assignedElements({ flatten: true }).filter((element) => element.tagName === "CALCITE-ACTION");
  ...
  // --------------------------------------------------------------------------
  async setFocus() {
    dom.focusElement(this.open ? this.actionElements[0] || this.menuEl : this.menuButtonEl);
  }
  // --------------------------------------------------------------------------
  //
  //  Component Methods
  ...
  renderMenuButton() {
    ...
    const actionNode = (index.h("calcite-action", { active: open, "aria-controls": menuId, "aria-expanded": open.toString(), "aria-haspopup": "true", class: CSS$1.menuButton, icon: ICONS.menu, id: menuButtonId, label: label, onClick: this.menuButtonClick, onKeyDown: this.menuButtonKeyDown, onKeyUp: this.menuButtonKeyUp, ref: this.setMenuButtonRef, text: label, textEnabled: expanded }));
    return dom.getSlotted(el, SLOTS$1.tooltip) ? (index.h("calcite-tooltip-manager", null, actionNode)) : (actionNode);
  }
  renderMenuItems() {
    const { actionElements, activeMenuItemIndex, open, menuButtonId, menuId, menuButtonEl, label, placement } = this;
    const activeAction = actionElements[activeMenuItemIndex];
    const activeDescendantId = (activeAction === null || activeAction === void 0 ? void 0 : activeAction.id) || null;
    return (index.h("calcite-popover", { label: label, open: open, placement: placement, referenceElement: menuButtonEl }, index.h("div", { "aria-activedescendant": activeDescendantId, "aria-labelledby": menuButtonId, class: CSS$1.menu, id: menuId, onKeyDown: this.menuActionsContainerKeyDown, onKeyUp: this.menuActionsContainerKeyUp, ref: (el) => (this.menuEl = el), role: "menu", tabIndex: -1 }, index.h("slot", null))));
  }
  render() {
    return (index.h(index.Host, null, this.renderMenuButton(), this.renderMenuItems(), index.h("slot", { name: SLOTS$1.tooltip })));
  }
